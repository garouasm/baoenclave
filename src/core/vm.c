...
static void vm_master_init(vm_t* vm, const vm_config_t* config)
{
    vm->master = cpu.id;
    vm->config = config;
...
void vm_copy_img_to_rgn(vm_t* vm, const vm_config_t* config,
...
void vm_map_img_rgn(vm_t* vm, const vm_config_t* config, struct mem_region* reg)
{
    if (reg->place_phys) {
...
void vm_init_mem_regions(vm_t* vm, const vm_config_t* config)
{
...
static void vm_init_ipc(vm_t* vm, const vm_config_t* config)
{
    ...
    for (int i = 0; i < config->platform.ipc_num; i++) {
        ...
        struct mem_region reg = {.base = ipc->base,
                                 .size = size,
                                 .place_phys = true,
                                 .phys = shmem->phys,
                                 .colors = shmem->colors};
...
vcpu_t* vm_init(vm_t* vm, const vm_config_t* config, bool master)
{
     ...
     */
    vcpu_t* vcpu = vm_vcpu_init(vm, config);
...
static inline emul_handler_t vm_emul_get(vm_t* vm, enum emul_type type,
                                         uint64_t addr)
{
